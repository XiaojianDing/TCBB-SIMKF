...
%
% Supported kernels:
%   'RBF_kernel'   : Gaussian RBF kernel
%   'RBF4_kernel'  : Higher-order RBF kernel
%   'sinc_kernel'  : Sinc kernel
%   'wav_kernel'   : Wavelet kernel
...
% Copyright (c) 2011,  KULeuven-ESAT-SCD
% Extended with linear and polynomial kernels
...
elseif strcmp(kernel_type,'poly_kernel')
    ...
    % kernel_pars = [degree, c]  (c 可选，默认为 1)
    degree = kernel_pars(1);
    ...
    if nargin < 4
        omega = (Xtrain * Xtrain' + c).^degree;
    else
        omega = (Xtrain * Xt' + c).^degree;
    end
end
end
