function [K1, K2] = Estimate_Number_of_Clusters_CIMLR(alldata, NUMC)

for i = 1:length(alldata)

    if i == 1
        for j=1:60
            D_Kernels(:,:,j)= kernel_matrix(alldata{i},'RBF_kernel', 2^(j));
        end
        ...
        for j=1:60
            D_Kernels= cat(3, D_Kernels,kernel_matrix(alldata{i},'RBF_kernel', 2^(j)));
        end
